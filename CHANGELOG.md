...
**Priority 0 security improvements based on comprehensive security audit:**

- **Node.js Version Enforcement**: Enforce Node.js >=22.12.0 for critical security patches (CVE-2025-59466, CVE-2026-21636).
  - Updated runtime guard minimum version from 22.0.0 to 22.12.0
  - Updated Dockerfile to pin exact Node.js 22.12.0-bookworm image
  - Enhanced startup error message to mention security patches

- **Docker Security Hardening**: Added comprehensive security flags to docker-compose.yml.
  - `security_opt: no-new-privileges:true` - prevents privilege escalation
  - `cap_drop: ALL` - drops all capabilities
  - `cap_add: NET_BIND_SERVICE` - adds only required capability for gateway
  - Resource limits: `mem_limit: 4g`, `cpus: 2.0`, `pids_limit: 200`

- **Filesystem Permission Protection**: Enhanced automatic permission fixes via `moltbot security audit --fix`.
  - Existing `fixSecurityFootguns()` already implements state directory (700) and config file (600) permissions
  - Automatic fixes for world-writable and group-writable critical files
  - Windows ACL support for proper permission enforcement on Windows systems

- **Security Audit System**: Comprehensive startup security checks.
  - `moltbot security audit` - check for common security footguns
  - `moltbot security audit --fix` - automatically fix permission issues
  - `moltbot security audit --deep` - includes live gateway probe
  - Critical findings cause immediate warnings with remediation steps
...
- `CHANGELOG.md` - Security changelog documentation

### Audit Reference
See `SECURITY_OPERATIONS_AUDIT.md` for complete security audit report.
